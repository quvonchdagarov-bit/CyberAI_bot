...
Tuzatilgan: logger import qo'shildi (kritik crash tuzatildi)
Yangilangan: Yaxshilangan hisobot, vaqt ko'rsatish
"""
...
from bot.loader import logger  # ← KRITIK: avval import qilinmagan edi!
...
async def handle_text_message(message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for url_value in urls[:settings.URL_ANALYSIS_LIMIT]:
            results.append(await analyze_url(session, normalize_url(url_value)))
    ...
    if results:
        ...
        short_text = format_short_result(best, "link", scan_ms)
        ...
        if best_score <= settings.GLOBAL_RISK_THRESHOLD:
            logger.info(
                "✅ Past xavf URL (%d <= %d) — ogohlantirish yuborilmadi",
                best_score, settings.GLOBAL_RISK_THRESHOLD,
            )
            try:
...
async def _send_analysis_result(
    ...
    if score <= settings.GLOBAL_RISK_THRESHOLD:
        logger.info(
            "✅ Past xavf matn (%d <= %d) — ogohlantirish yuborilmadi",
            score, settings.GLOBAL_RISK_THRESHOLD,
        )
        return
